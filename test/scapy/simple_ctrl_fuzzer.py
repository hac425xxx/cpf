...
def TestCtrlTransfer(device, rt, r, v, i):
    for size in (0, 10, 100, 4000):
        ...
        try:
            ...
            if (
                    e.backend_error_code != -9 and e.backend_error_code != -1):  # ignore LIBUSB_ERROR_PIPE and LIBUSB_ERROR_IO
        ...
        try:
            ...
            print 'IN  %0.2x %0.2x %0.4x %0.4x data(%u) len(%u):\t%s' % (
                rt, r, v, i, len(res), size, binascii.hexlify(res))
        except usb.core.USBError as e:
            if (
                    e.backend_error_code != -9 and e.backend_error_code != -1):  # ignore LIBUSB_ERROR_PIPE and LIBUSB_ERROR_IO
